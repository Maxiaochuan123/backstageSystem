...
export default{
  ...
  disabledRegion(that, scope){
    this.public_Disabled(that, api.disabledRegion, {geoId: scope.id}, scope.isEnable == 1 ? '禁用' : '启用', api.getRegion);
  },
  ...
  deleteMechanism(that, scope){
    this.public_Delete(that, api.deleteMechanism, {officeId:scope.id}, api.getMechanism);
  },
  // 启用 / 禁用
  disabledMechanism(that, scope){
    this.public_Disabled(that, api.disabledMechanism, {officeId:scope.id}, scope.isEnable == 1 ? '禁用' : '启用', api.getMechanism);
  },
  ...
  disabledRole(that, scope){
    this.public_Disabled(that, api.disabledRole, {roleId:scope.id}, scope.isEnable == 1 ? '禁用' : '启用', api.getRole);
  },
  // 删除 角色
  deleteRole(that, scope){
    this.public_Delete(that, api.deleteRole, {roleId:scope.id}, api.getRole);
  },
  ...
  disabledUser(that, scope){
    this.public_Disabled(that, api.disabledUser, {userId:scope.id}, scope.isEnable == 1 ? '禁用' : '启用', api.getUser);
  },
  ...
  //删除 公共方法
  public_Delete(that, apiMethod, deleteData, getList){
  ...
  public_getList(that, apiMethod){
    ...
    apiMethod(that.paging.req).then(res=>{
      if(res.code == '200'){
        ...
        that.paging.totalPage = res.data.totalPages;
      }else{
